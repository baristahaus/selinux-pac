...
| **dontaudit** | A rule that silently allows an access without writing an AVC record. Used by the vendor policy to silence noisy tuples; rarely written in a module because it hides failures. | [When Policy Says No](repo:book/part2/12-when-policy-says-no.md) |
...
| **gen_context** | The `.fc` directive that declares a context for every file under a path (`gen_context(file, s0)` or per-path). `restorecon` uses this as its source of truth. | [File Contexts and the Label Lifecycle](repo:book/part2/09-file-contexts-and-the-label-lifecycle.md) |
...
| **sepolgen** | `sepolicy generate` — suggests a rule from the running policy; each suggestion is a candidate for review, never a copy-paste. | [Querying the Installed Policy](repo:book/part2/07-types-attributes-and-classes.md) |
...
| **spc_t** | A vendor-provided system process type — `system_u:system_r:spc_t:s0` is the standard label for the policy management daemons. Your module rarely touches it. | [Types, Attributes and Classes](repo:book/part2/07-types-attributes-and-classes.md) |
...
| **typeattribute** | A line that adds a type to an attribute — `typeattribute <attr> <tgt_t>;` — used to build the named collections that allow rules target. | [Types, Attributes and Classes](repo:book/part2/07-types-attributes-and-classes.md) |
