...
The host tells you whether it is enforcing, permissive, or permissive-system. These
...
| `cat /etc/selinux/config` | Shows the `SELINUX=` and `SELINUXTYPE=` values the host will use on the next boot. It is never `getenforce`'s answer | `rhel host`, `root` |

:::: why Each of these answers a different question
`getenforce` is "is it currently blocking", and `sestatus` is "what is the policy version".
`semanage permissive -l` is "which domains are we tolerating", and `/etc/selinux/config` is
"what will we be on reboot". They answer different slices of the same truth, and they routinely
disagree, which is the point: `setenforce 0` leaves `getenforce` saying `Permissive`
...
| `matchpathcon <path>` | "The label policy assigns this path by default", per `/etc/selinux/<type>/contexts/files/`. That is the answer you compare to what is on the box. `-m <kind>` forces the lookup for a file type (`file`, `dir`, `pipe`, `chr_file`, `blk_file`, `lnk_file`, `sock_file`). `-V` checks the context on disk against the default and prints only the mismatches | `rhel host` |

:::: note `ls -Z` is the one command you never forget
Because every access decision names the label of the object, `ls -Z` is the most useful command
...
These commands repair the gap between the label a path *must* have and the label it *has*.
...
`fixfiles relabel` re-labels *every* mounted filesystem, in place, while the host runs. It
...
drifted at scale. For a single path, use `restorecon`.
:::
...
production host carries `-P`, and every lab host carries `-P`.
:::

## Modules — compile, install, remove

These commands each own a different step of the policy module lifecycle. Each invocation
is checked against `scripts/compile_and_validate.sh` and `scripts/lib/compile_policy.sh`.
...
| `semodule -r <name>` | Removes a module from the running policy and unloads its rules | `rhel host`, `root` |
...
| Command | What it tells you or changes | Needs |
|---|---|---|
| `sesearch -A -s <src_t> -t <tgt_t> -c <class> -p <perm>` | Searches the running policy for matching `allow` rules and returns each rule verbatim | `rhel host`, `root` |
| `seinfo -a <attribute> -x` | Lists every type that carries `<attribute>`. It reads the binary policy, either the running one or a `--policy <policy.kern>` file, and ships in `setools-console`, the same package as `sesearch` | `rhel host`, `root` |
...
`sepolgen-ifgen` builds. That is the same path `cli/deterministic_gen.py` takes. If the database
is missing, the generator says so and refuses base-type AVCs unless you pass
`--allow-degraded`. It never guesses an interface from the running policy.
:::
...
| `audit2why` | Names the cause of each denial: a missing type-enforcement allow, a wrong label, a boolean, or a port. It then tells you to use `audit2allow`. It does not print rules | `rhel host`, `root` |
| `audit2allow` | Converts each AVC into an allow rule verbatim. The output is a suggestion that needs review, and you must never pipe it directly into `semodule -i` | `rhel host`, `root` |

::: warn `audit2allow` is the most dangerous command in the book
The output of `audit2allow` is a raw `allow` per AVC, and it is broad enough that a single run can
turn `myapp_t` into `unconfined_t`. Read it. Review it. Never pipe it into `semodule -i`.
This book's generator produces the same rule set, but `validate_forbidden_patterns.sh` gates it.
:::

## This repository's own entry points

Each invocation is checked against the script or Makefile that owns it. No inferred flags,
no guessed defaults.

| Command | What it tells you or changes | Needs |
|---|---|---|
| `make check` | Offline health check: `test` (fixtures + static + smoke), `lint`, `book-check`. No SELinux host is required, but the `deps` step wants `pip3`, and the linters skip themselves when absent | `laptop` |
...
| `make book-check` | Checks the manual (internal links, anchors, `repo:` references) via `python3 tools/book/build.py --check` | `laptop` |
| `bash scripts/dev_generate_policy.sh` | Developer self-service: export AVCs → run `cli/deterministic_gen.py` → generate policy → diff → promote into `selinux/`. It supports `--apply`, `--tune-report`, `--force REASON`, `--allow-needs-review`, `--skip-export`, `--open-pr` | `build host` (for promote) / `laptop` (for explain) |
...
| `ansible-playbook` (canary) / `ansible-playbook` (soak) | Deploys the canary module (`part4/19-canary-soak-enforce.md`) or runs the daily soak monitor. `soak_monitor.yml` calls `monitor_avc.sh --manifest … --max-net-new …`, and `soak_status.yml` calls `collect_soak_facts.sh`. (`check_soak_ready.sh` is the host-side CLI for the same checks. The role itself runs `collect_soak_facts.sh`) | `controller`, `rhel host` |
...
A failing fixture is the same signal as a failing test: CI fails, and the PR is blocked.
:::

## What you can do now

- Pick one host, and run `getenforce`, `sestatus`, `id -Z`, and `ls -Z /var`. You now own the
