...
|---|---|---|
| **Enforcing** | `setenforce 1` or `SELINUX=enforcing` in `/etc/selinux/config` | denials block. AVCs are written with `permissive=0` |
...
| **Disabled** | `SELINUX=disabled` in `/etc/selinux/config`, then unload policy | no LSM hooks fire. No AVCs at all |
...
The most common bad answer in an incident is `setenforce 0`: permissive for the whole host. The most damaging bad answer is `SELINUX=disabled`. The difference is not cosmetic. It is structural.
...
The permissive list is stored in the kernel's live policy DB. Every hook on the host consults it: a file access, a socket bind, or a `fork`. For whichever operation the hook mediates, if the source domain is on the list, the answer is `permissive=1`. Off the list, every hook answers `permissive=0`.
...
Nothing here changes state. The commands only read.
...
- **Name every state a host can be in.** Enforcing, domain permissive, system permissive, disabled. Each is reachable with one command or one configuration line.
- **Read the two checks every on-call run.** `getenforce` answers the whole-system state. `sudo semanage permissive -l` answers the per-domain state. Both of them are required, and each of them is independent.
...
- **Name what each command does at the kernel hook.** `sudo semanage permissive -a` adds a domain to the permissive list. Every hook on that domain then answers `permissive=1`. `sudo semanage permissive -l` lists every domain currently on the list. `sudo semanage permissive -d` removes a domain. Every hook on that domain then answers `permissive=0` again.
- **Read permissive as "this breaks in Enforcing."** A permissive service looks healthy while guarded operations are denied. Zero HTTP 500s during soak is not evidence the policy is correct. The policy can be right about every HTTP path and wrong about every logrotate run.
...
- **Name the soak gate.** The gate needs four things: marker age ≥ 7 days, and a net-new access count of zero. It also needs events within `soak_max_avc` and a signed-off deploy report. Then `check_soak_ready.sh` prints the pass line, and `enforce.yml` flips the domain back to enforcing. The deploy report records that every endpoint still returned 200.
...
Chapter 6 moves to the policy language itself. It covers the files that make a module and the type system underneath them. It also covers the decisions that separate a reviewable rule from a dangerous one.
