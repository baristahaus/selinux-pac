...
> Every policy module is a list of types. Review that list: *why each type exists,
> what happens if a rule names a generic type instead, and how the attribute shortcuts collapse
> ten rules into one*. That review is the single most valuable thing you can learn about
> the module language.
...
That is the only syntax in `.te` that creates a type. From that moment, the type appears
in allow rules, in file-context lines, and in `sesearch` queries. The type is the thing
all three tools talk about.

In the example module (`selinux/myapp.te`), every line of type declarations is a purpose
statement. Each type is tied to a path, a port, or a role. Eleven types are declared for
a single application, and each one is *owned* by its purpose:
...
The naming convention is plain: `myapp_<purpose>_t`. The `myapp_` prefix scopes the type
to the module. The middle word names its purpose. The `_t` suffix reminds you that every
label field ends in `t`. The convention is not decorative. It is the reason `sesearch`
...
A generic type carries the policy of every consumer. `var_t` already has rules for
`rsyslog_t`, `cron_job_t`, `logrotate_t`, `root`, and a dozen other domains. Adding your own `allow
myapp_t var_t:file write;` gives your process access to every directory the generic type
applies to. The kernel sees: *every file in `/var` can be written*. That is not only your
application's files.
...
That line says: *every rule written against `file_type` now covers this type.* The kernel does
not gain new semantics. The `file` object class was always `file`. But the policy author gains
a shorthand. Every interface that takes an *attribute* targets every tagged type at once:
...
Each of these lines declares the type and attaches it to an attribute family. The base
policy's file, pid-file, port, and log rules now cover these types, so a handful of
declarations stands in for a rule per type. `read_files_pattern` covers every file-tagged
type. The pid-file rules (`files_search_pids` and friends) cover every PID-tagged type. The
domain can bind every port-tagged type once it has `name_bind`. `logging_log_filetrans` and
the log-management patterns cover every log-tagged type.

The attribute is how an interface writes rules for *all* types of its class, not just one.
The author writes one `typeattribute` line, and the interface writes all the allows. This is
the difference between "ten rules per type" and "one rule per class."

### How the interface macros read

The macros in the example module are not custom. They are refpolicy interfaces, and each
pairs an attribute with a predictable set of allows. These are the ones you will see most often:
...
| `init_daemon_run_dir(TYPE, NAME)` | the run-directory association for the service | the init domain can create the runtime path for `NAME`. The module writes the domain's own access to it separately |
...
`system/init.if`) on any host with `selinux-policy-devel` installed. The build writes the
`allow` rules that use these names. The author writes the declarations. `seinfo -a` lists the
attributes that actually exist in the policy you are running.

## Object classes and permissions

The *class* decides what kinds of operations are even meaningful. The *permission* names one
of them. A rule is the triple: *allow source target:CLASS { permission, … }*.

The kernel already names the class in every AVC line, in the `tclass` field. So the list below
is what a service author will meet in practice, taken from the base policy object-class definitions:
...
The most important column is the third: the permissions that appear in *every* denial. A
process needs `file` permissions to read data. It needs `dir` permissions to create new
data. It needs `tcp_socket` permissions to talk to a port. It needs `process` permissions
to spawn a child. `allow myapp_t myapp_var_lib_t:file write;` is only correct if the
process writes text into an existing file and does not create a new one. When it creates a
new file, `dir:write` (or the `manage_files_pattern` macro) is the real allow.
...
The first query is the most useful. It answers "what does the OS already let me do?" If the
row already exists, you do not need to add it. If it does not, you need to add the type, so a
row can be written against it. Or you accept that your application does not reach that
resource.
...
`seinfo -t` and `seinfo -a` are for exploration. They show the *installed* policy: the union
of the base policy plus all loaded modules. They are the most honest source of truth when an
...
The review of a target type asks one question: *does the type name match the path, or does it
match a category?* Every chapter from Part III returns to that question. The constants in
...
Each constant has a role. `FORBIDDEN_TARGET_TYPES` names types the module is forbidden to
talk to. `GENERIC_FILE_TYPES` names file types that every other service already reaches.
`GENERIC_PORT_TYPES` names ports that every networked service already reaches. The verdict
per constant:
...
The pattern against a broad port grant is not named in that script. The classification lives
in `cli/deterministic_gen.py`, with the constant sets in `cli/policy_rules.py`. An allow whose
target is one of the generic port types (`unreserved_port_t`, `port_t`, `reserved_port_t`,
`ephemeral_port_t`) and whose permission set includes `name_bind` is classified as
`private_port`, with `next_action: add_manifest_port`. The port type is the fix, not the
allow. `direct` is a different verdict, for the allows that name the module's own private
types.

The `validate_policy_semantics.sh` script goes further. It asserts on the compiled module
that no `allow` reaches `shadow_t` or `unlabeled_t`. It also asserts that every `entrypoint`
allow names only a type under the module's own prefix:
...
These assertions prove that the type system does the work the author forgets. They refuse a
grant against a privileged type or an unlabeled object *before* it reaches review.
...
The first query shows the universe of types on your host: the base policy plus all modules.
The second shows the attributes every type can be tagged with. The third shows your own
module's allow rows. Read the rows against generic types first. They are the ones the review
asks about.
:::
...
| list my own allows on a class | `sesearch -A -s myapp_t -c file` | the rules already written, so nothing to add |
| list allows against a target type | `sesearch --allow -t myapp_var_lib_t` | the scope of your type's allow rows |
| check a deny before adding a rule | `sesearch -A -s myapp_t -c file` | you see whether the row already exists |
...
If it matches a category, the rule is a category grant. It is still correct, but it is the
kind of grant you review second. If it matches a privileged type, the rule is forbidden. You
do not review it. You rewrite it.
