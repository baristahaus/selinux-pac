...
The service kept running. The denial is a soak finding: a net-new access need that slipped past the gate. This is the better shape. The OS is still enforcing, the app still answers, and the wrong answer is a PR, not a hotfix.

`soak_monitor.yml` flags net-new access needs on the canary group. A finding does not cancel the soak. The domain is still on the permissive list, because the canary was supposed to surface exactly this. The operator does not go to the box. The operator pulls the AVCs, the controller generates, a CODEOWNER reviews, and the RPM ships.

The panic from the app team in this shape is the wrong one: it is the expectation of a fix this hour. The answer you give is the expectation of a fix this week, with a canary clock that resets on deploy.

### The shape A decision table

| Symptom | First check | Decision | Who decides | What must never be done |
|---|---|---|---|---|
| Service responding, AVC in audit.log, domain permissive | `getenforce` → Enforcing. `semanage permissive -l` → domain still listed. `ausearch -m avc -ts recent` → denials but no outage | Soak still running. Denial is a finding, not an outage. | SELinux admin, with the change-ticket owner's sign-off on the PR | `setenforce 0` on the host, which makes the whole OS permissive |
| `soak_monitor.yml` ran daily, flagged net-new count | `soak_status.yml` or the AAP **SELinux – Soak monitor** job status, and the deploy report at `/var/lib/myapp/selinux_deploy_report.json` | Net-new count over threshold, with `soak_use_net_new` still on. The gate is informational, not blocking | Same as the first row | `audit2allow | semodule -i` on production |
| Application still accepting traffic | `semanage permissive -l` still contains the domain. AVCs are present, but the path is reachable | PR path: generate from the AVCs, CODEOWNER review, signed RPM, re-canary | The SELinux admin owns the PR. The change-ticket owner approves the PR | Editing `/etc/selinux/config`, which is a persistent OS state change |
...
The playbook is [`repo:ansible/emergency_rollback.yml`](ansible/emergency_rollback.yml). It is the AAP template `SELinux – Rollback`. It is the operator's emergency card. It does not downgrade the host, and it does not disable the kernel. It puts the domain back on the permissive list, optionally downgrades the RPM, and captures the AVCs for the PR path that follows.
...
| **2. Fallback add** | `Permissive relief via semanage when module unavailable`: `semanage permissive -a {{ domain }}` | The same result if the host is missing the collection. |
...
| **4. Dontaudit baseline restore** | `Restore dontaudit baseline after rollback`: `semodule -B` | The dontaudit baseline of the host is restored, so the next canary has visibility again. |
...
| **6. Services restarted** | `Restart application services after rollback`: systemd `state: restarted` | The domain is permissive, and the binaries are running again. |
| **7. Runtime contexts restored** | `restorecon -Rv {{ runtime_dir }}` | The runtime directory, relabeled after the service started. |
...
| **9. Endpoints check (optional)** | `Wait for endpoints when ops package available`: `wait_for_endpoints.sh` against the manifest | A sanity probe. It is not guaranteed when the ops RPM is missing. |
...
| **11. Soak clock reset** | `Record rollback timestamp`: `date +%s > {{ soak_marker_file }}` | The clock is reset. Soak starts over from zero. This is why the timeline for shape B is longer than the one for shape A. |

The operator must supply two things. The domain name comes from the inventory (`hosts.all.selinux_pac.domain`). The RPM downgrade version (`rollback_dnf_version`) is the only value the playbook needs, and it drives the optional branch. Without it, the playbook skips the downgrade step. Every other step still runs, and the permissive relief from step 1 is what restores service at once.
...
| RPM downgrade | the policy is a known version, a step-back to an earlier checked state | the root cause |
| `semodule -B` | the dontaudit baseline of the host, so future canaries can see denials again | the state of the application |
| `restorecon` | on-disk labels realigned to the `.fc` rules | the code path that produced the denial |
| Service restart | the domain runs under permissive mode, and HTTP probes pass again | the new rule that needs to be added |
| Soak clock reset | the clock starts over, so the next cycle is a fresh soak | the urgency |
...
| **Lab soak window refused on production** (line 9–16) | `soak_min_days` below 7 on a host in `group: production` is rejected. It is a lab-only value. |
| **Soak gate skipped when `force_enforce=true`** (lines 19–34) | The role does not run `collect_soak_facts.sh`, does not collect `soak_facts`, and does not pass the gate. |
| **Canary marker missing** (lines 36–41) | Still blocked without the marker. `deploy_canary` must run before `force_enforce`. |
| **Soak period not met** (lines 43–50) | Days elapsed below `soak_min_days`. The role skips this only if `force_enforce` is set. |
...
After the gate is skipped, the role does four things. It re-enables the dontaudit baseline with `semodule -B`. It removes the permissive flag, with `semanage permissive -d`, or with `semodule -r {{ stub_module_name }}` on the FCOS overlay. It checks and restarts the services. Then it writes the enforce deploy report. The `rescue:` block puts the domain back to permissive if any of those steps fails. So the operator keeps a safety net even when the gate is gone.
...
| Item | Why |
|---|---|
| **URL of the ticket** | `https://github.com/anurag-saran/selinux-pac`. Every fix lives as a reviewed commit on this repo. There is no host-level patch. |
| **Timestamp** | the AVC in `audit.log`, and the time the denial was logged. The app team needs this for correlation. |
| **Request** | "check that you have a CODEOWNER account and access to the policy repo." Without a reviewer, the PR never ships. |
...
|---|---|
| "it is fixed this hour" | the ETA for shape A is a PR. The ETA for shape B is a rollback. Neither is an hour. |
| "we can keep it permissive forever" | the gate closes. `collect_soak_facts.sh` runs, net-new hits the threshold, and `force_enforce` is the only escape. |
| "no data loss from the denial" | permissive=1 still denies. The data path is silent until it is not. |
| "the app is back up" | The app in shape A is already answering. Only the app in shape B is failing. |
...
| Evidence | Where | How |
|---|---|---|
| **Filtered AVCs** | `/tmp/emergency_avc.log`. `emergency_rollback.yml` step 8 writes it, or the operator writes it on the host. | `ausearch -m AVC,USER_AVC,SELINUX_ERR,USER_SELINUX_ERR -ts recent --format raw` filtered to `{{ domain }}` and persistent paths. `generate_emergency_patch.yml` requires this log to be present. |
...
| **The PR** | `policy_out/` directory. Only the controller writes it. | `generate_emergency_patch.yml` runs `deterministic_gen.py` against the AVC log and the manifest, and writes `findings.json`, `avc_summary.txt`, `pr_summary.md`, and the `.te`/`.fc` patch. The LLM polish is optional, and it needs `OPENAI_API_KEY` set for prose. The result never goes on prod. |
...
|---|---|
| **Was the denial new or newly enforced?** | A denial that exists under permissive is a soak-fail, because the app kept running. A denial that first appears when the domain leaves the permissive list is an outage. This is the question that separates the two shapes. |
| **Did the canary miss the code path?** | `canary_max_avc=0` blocks a canary with denials already present, and a zero override needs explicit approval. If the canary ran clean, the path was not exercised, or the dontaudit baseline was on and swallowed the denial. |
| **Why did the soak not see it?** | Net-new count against the installed policy (`sesearch`), or the raw AVC count without `sesearch`. The soak fails closed when `sesearch` is missing, and the gate is informational when it is present. |
| **What would have caught it earlier?** | a probe on staging, a soak day with the right `max-net-new` threshold, or a test fixture under `tests/fixtures/blast_radius/` that classifies the delta correctly. |

The question from the app team is "why was it enforcing with no warning?" The answer is the change ticket. The gate was satisfied, the ticket is the approval, and the rollback is the apology.
...
`setenforce 0` restores service in seconds and costs a year. The cost is the relabel on the way back. It is a host that hides denials during the next incident. It is an audit trail that stops where it became interesting. Every other row has the same shape: fast, local, invisible in the repository.
:::

::: try Rehearse the incident
Laptop: walk the decision table above against the `01-mislabeled-var-lib` fixture (`docs/examples/fixtures/deterministic/01-mislabeled-var-lib/avc.log`). It pins shape A: a denial while the domain is permissive, the path reachable, and the answer a pull request.

Two-host lab (controller only): reset the VMs with `bash scripts/reset_demo_vms.sh`. Work through Part 1 of `docs/admin/203-RHEL_TWO_HOST.md`, and deploy the canary. Then break `/feature-spool`, trigger `ansible/emergency_rollback.yml`, and check that the deploy report reads `phase: rollback`. Everything before that last command is read-only: `getenforce`, `semanage permissive -l`, `ausearch -m avc -ts recent`, `ls /var/lib/myapp/selinux_canary_deployed_at`.
:::
...
- **Roll back** with the playbook, not the keyboard. The app domain goes back to permissive first, and the RPM downgrade is a separate decision.
